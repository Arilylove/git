$(function () {
    $("#jqGrid").jqGrid({
        url: host + "/sys/additionalFeeScheme/listByPage.do",
        datatype: "json",
        xhrFields : {
        ...
        crossDomain : true,
        colModel: [			
			{ label: '附加费方案ID', name: 'additionalFeeSchemeCode', index: "id", width: 40, key: true ,hidedlg:true,hidden:true},
			{ label: '附加费方案名称', name: 'additionalFeeSchemeName', width: 60 },
			{ label: '状态', name: 'state', width: 50, formatter: function(value, options, row){
				if(value == '0'){
				...
				if(value =='2'){
					return '<span class="label label-warning">系统</span>';
				}
			}},
